...
void ComputerHard(int& c1,int& c2,int& c3,int& c4,int& c5,
    ...
    // Se il tabellone è pieno, esci
    if(TabellonePieno(c1,c2,c3,c4,c5,c6,c7,c8,c9)) return;

    // 1% di errore (mossa casuale)
    if((rand() % 100) < 0.1) {
        bool mossa = false;
        while(!mossa) {
            int scelta = rand() % 9 + 1;
            if(scelta==1 && c1==0){ c1=simboloC; mossa=true; }
            if(scelta==2 && c2==0){ c2=simboloC; mossa=true; }
            if(scelta==3 && c3==0){ c3=simboloC; mossa=true; }
            if(scelta==4 && c4==0){ c4=simboloC; mossa=true; }
            if(scelta==5 && c5==0){ c5=simboloC; mossa=true; }
            if(scelta==6 && c6==0){ c6=simboloC; mossa=true; }
            if(scelta==7 && c7==0){ c7=simboloC; mossa=true; }
            if(scelta==8 && c8==0){ c8=simboloC; mossa=true; }
            if(scelta==9 && c9==0){ c9=simboloC; mossa=true; }
        }
        return;
    }

    // --- Prima prova a vincere se possibile ---
    if (c1 ==simbolop && c9 ==simbolop && c5 == 0 || c3 == simbolop && c7 == simbolop && c5 == 0) {
        c5 = simboloC;
        return;
    }
    else {
        if(c1==simboloC && c2==simboloC && c3==0){ c3=simboloC; return; }
        if(c1==simboloC && c3==simboloC && c2==0){ c2=simboloC; return; }
        if(c2==simboloC && c3==simboloC && c1==0){ c1=simboloC; return; }

        if(c4==simboloC && c5==simboloC && c6==0){ c6=simboloC; return; }
        if(c4==simboloC && c6==simboloC && c5==0){ c5=simboloC; return; }
        if(c5==simboloC && c6==simboloC && c4==0){ c4=simboloC; return; }

        if(c7==simboloC && c8==simboloC && c9==0){ c9=simboloC; return; }
        if(c7==simboloC && c9==simboloC && c8==0){ c8=simboloC; return; }
        if(c8==simboloC && c9==simboloC && c7==0){ c7=simboloC; return; }

        if(c1==simboloC && c4==simboloC && c7==0){ c7=simboloC; return; }
        if(c1==simboloC && c7==simboloC && c4==0){ c4=simboloC; return; }
        if(c4==simboloC && c7==simboloC && c1==0){ c1=simboloC; return; }

        if(c2==simboloC && c5==simboloC && c8==0){ c8=simboloC; return; }
        if(c2==simboloC && c8==simboloC && c5==0){ c5=simboloC; return; }
        if(c5==simboloC && c8==simboloC && c2==0){ c2=simboloC; return; }

        if(c3==simboloC && c6==simboloC && c9==0){ c9=simboloC; return; }
        if(c3==simboloC && c9==simboloC && c6==0){ c6=simboloC; return; }
        if(c6==simboloC && c9==simboloC && c3==0){ c3=simboloC; return; }

        if(c1==simboloC && c5==simboloC && c9==0){ c9=simboloC; return; }
        if(c1==simboloC && c9==simboloC && c5==0){ c5=simboloC; return; }
        if(c5==simboloC && c9==simboloC && c1==0){ c1=simboloC; return; }

        if(c3==simboloC && c5==simboloC && c7==0){ c7=simboloC; return; }
        if(c3==simboloC && c7==simboloC && c5==0){ c5=simboloC; return; }
        if(c5==simboloC && c7==simboloC && c3==0){ c3=simboloC; return; }

        // --- Poi blocca il giocatore se può vincere ---
        if(c1==simbolop && c2==simbolop && c3==0){ c3=simboloC; return; }
        if(c1==simbolop && c3==simbolop && c2==0){ c2=simboloC; return; }
        if(c2==simbolop && c3==simbolop && c1==0){ c1=simboloC; return; }

        if(c4==simbolop && c5==simbolop && c6==0){ c6=simboloC; return; }
        if(c4==simbolop && c6==simbolop && c5==0){ c5=simboloC; return; }
        if(c5==simbolop && c6==simbolop && c4==0){ c4=simboloC; return; }

        if(c7==simbolop && c8==simbolop && c9==0){ c9=simboloC; return; }
        if(c7==simbolop && c9==simbolop && c8==0){ c8=simboloC; return; }
        if(c8==simbolop && c9==simbolop && c7==0){ c7=simboloC; return; }

        if(c1==simbolop && c4==simbolop && c7==0){ c7=simboloC; return; }
        if(c1==simbolop && c7==simbolop && c4==0){ c4=simboloC; return; }
        if(c4==simbolop && c7==simbolop && c1==0){ c1=simboloC; return; }

        if(c2==simbolop && c5==simbolop && c8==0){ c8=simboloC; return; }
        if(c2==simbolop && c8==simbolop && c5==0){ c5=simboloC; return; }
        if(c5==simbolop && c8==simbolop && c2==0){ c2=simboloC; return; }

        if(c3==simbolop && c6==simbolop && c9==0){ c9=simboloC; return; }
        if(c3==simbolop && c9==simbolop && c6==0){ c6=simboloC; return; }
        if(c6==simbolop && c9==simbolop && c3==0){ c3=simboloC; return; }

        if(c1==simbolop && c5==simbolop && c9==0){ c9=simboloC; return; }
        if(c1==simbolop && c9==simbolop && c5==0){ c5=simboloC; return; }
        if(c5==simbolop && c9==simbolop && c1==0){ c1=simboloC; return; }

        if(c3==simbolop && c5==simbolop && c7==0){ c7=simboloC; return; }
        if(c3==simbolop && c7==simbolop && c5==0){ c5=simboloC; return; }
        if(c5==simbolop && c7==simbolop && c3==0){ c3=simboloC; return; }
    }
    // --- Se nessuno può vincere, fai mossa casuale ---
    bool mossa = false;
    while(!mossa)
    {
        int scelta = rand() % 9 + 1;
